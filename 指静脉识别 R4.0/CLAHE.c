	...
	*/
{
	...
	uchar aLUT[256];        /* lookup table used for scaling of input image */
...
IplImage* CLAHEMain(IplImage* src)
{
	int Height = src->height;
	int Width = src->width;
	uchar ImageArray[320][240] = { 0 };
	IplImage* CLAHEImage = cvCreateImage(cvSize(Width, Height), IPL_DEPTH_8U, 1);
	...
	for (int i = 0; i < Height; i++)
	{
		...
		{
			ImageArray[i][j] = ptr[j];
		}
	}
	CLAHE(ImageArray, 240, 320, 0, 255, 8, 8, 256, 3);
	for (int i = 0; i < Height; i++)
	{
		for (int j = 0; j < Width; j++)
		{
			CV_IMAGE_ELEM(CLAHEImage, uchar, i, j) = ImageArray[i][j];
		}
	}
	return CLAHEImage;
}